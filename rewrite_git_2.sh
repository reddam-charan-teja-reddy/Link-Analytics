...
git add .
git commit -m "Temp backup before rewrite 3" || true
git branch temp-backup-branch-3 || true

git checkout --orphan new-history-3
git rm -rf --cached .
...
# Safe replacement: Reset main branch to point to new-history-3 commits
git checkout main
git reset --hard new-history-3
